...
// BCMA Bedside Transfusion Verification — Phase 11
const BloodBankService = require('../services/BloodBankService');

const bedsideVerifyAndStartTransfusion = asyncHandler(async (req, res) => {
    const hospitalId = getHospitalId(req);
    const { patientUhid, isbtDinScanned, wardId, bedNumber, rateMlPerHour, vitalsBaseline } = req.body;

    if (!patientUhid || !isbtDinScanned) {
        return ResponseHandler.error(res, 'Both patientUhid and isbtDinScanned are required.', 400);
    }

    const result = await BloodBankService.bedsideVerifyAndStartTransfusion(
        String(patientUhid),
        ...
        hospitalId,
        {
            ...
            rateMlPerHour: rateMlPerHour || 100,
            vitalsBaseline: vitalsBaseline || {},
        }
    );
    ...
    return res.status(201).json({
        ...
        message: result.message,
        data: {
            transfusionId: result.transfusionId,
