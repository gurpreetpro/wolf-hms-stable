...
const getProblems = asyncHandler(async (req, res) => {
    ...
    const result = await pool.query(
        'SELECT * FROM problem_list WHERE patient_id = $1 AND (hospital_id = $2) ORDER BY status, priority, created_at DESC',
...
const deleteProblem = asyncHandler(async (req, res) => {
    ...
    const result = await pool.query('DELETE FROM problem_list WHERE id = $1 AND (hospital_id = $2) RETURNING id', [id, hospitalId]);
