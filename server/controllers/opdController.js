...
const registerOPD = asyncHandler(async (req, res) => {
    ...
    if (patientRes.rows.length > 0) {
        ...
        try {
            ...
            // [409 GRACEFUL HANDLING] Unique constraint violation — concurrent registration with same phone
            if (insertErr.code === '23505') {
                console.log(`[OPD] Graceful 409: Phone ${phone} registered concurrently. Returning existing.`);
                const existingPatientRes = await pool.query(
                    'SELECT id, name, phone, uhid, global_uhid, dob, gender, abha_id FROM patients WHERE phone = $1 AND hospital_id = $2 LIMIT 1',
                    [phone, hospitalId]
                );
                if (existingPatientRes.rows.length > 0) {
                    return ResponseHandler.success(res, {
                        patient: existingPatientRes.rows[0],
                        message: 'Patient with this mobile number is already registered in the queue.',
                        isDuplicate: true
                    }, 'Patient already registered', 200);
                }
            }
            throw insertErr; // Re-throw non-conflict errors
        }
    ...
    try {
        if (!docId) {
            // Query first active doctor matching hospital_id
            docRes = await pool.query(
                "SELECT id, username, department, consultation_fee FROM users WHERE role = 'doctor' AND is_active = true AND hospital_id = $1 ORDER BY id ASC LIMIT 1",
                [hospitalId]
            );
            if (docRes.rows.length === 0) {
                return ResponseHandler.error(res, 'No active doctors available for this hospital branch', 400);
            }
            docId = docRes.rows[0].id;
        } else {
            // Verify provided doctor_id is active and belongs to the hospital
            docRes = await pool.query(
                "SELECT username, department, consultation_fee FROM users WHERE id = $1 AND role = 'doctor' AND is_active = true AND hospital_id = $2",
                [docId, hospitalId]
            );
            if (docRes.rows.length === 0) {
                return ResponseHandler.error(res, 'No active doctors available for this hospital branch', 400);
    ...
    try {
        ...
        if (err.code === '23503' && err.message.includes('doctor_id')) {
            return ResponseHandler.error(res, 'No active doctors available for this hospital branch', 400);
        }
        throw err;
    }
