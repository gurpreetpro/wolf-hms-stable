...
const ENDPOINTS = [
  ...
  // Maternity
  { path: '/api/maternity/antenatal/{PID}', label: 'Maternity Antenatal' },
  { path: '/api/maternity/partograph/{PID}', label: 'Maternity Partograph' },
  { path: '/api/maternity/deliveries/{PID}', label: 'Maternity Deliveries' },
  ...
  { path: '/api/lab/trends/{PID}', label: 'Lab Patient Trends' },
  ...
  { path: '/api/finance/ledger/{PID}', label: 'Finance Patient Ledger' },
  ...
  // IPD Patient Portal
  { path: '/api/ipd/my-stay/{PID}', label: 'IPD My Stay' },
...
async function probeEndpoint(ep) {
  ...
  if (streamTimeout) {
    return { endpoint: ep, status: 0, elapsed, passed: true, crash: false, sseTimeout: true, note: 'SSE/stream - expected, not a crash' };
  }
...
function generateReport(results) {
  ...
  const sseStreams = results.filter((r) => r.sseTimeout);
  const passed = results.filter((r) => !r.crash && !r.sseTimeout);
